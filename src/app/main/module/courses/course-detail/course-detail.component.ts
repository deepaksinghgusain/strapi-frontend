...
export class CourseDetailComponent implements OnInit, OnDestroy {
        ...
        this.unsubscribe$.add(
          this.cartService.getCart(this.cartId).subscribe({
            next: (resp: any) => {
              ...
              this._commannService.setCartQtyObs(
                resp?.data?.attributes?.CartItem.length
              );
            },
            error: (res: any) => {
  ...
  getCoursePageData() {
    this.unsubscribe$.add(
      this.pageService.getCourseDetailPage().subscribe((res: any) => {
        this.heroImageSection = res?.data[0]?.attributes?.blocks.filter(
          (res: { __component: string }) =>
            res.__component === 'blocks.hero-image-with-button'
        )[0];
        this.apiSection = res?.data[0]?.attributes?.blocks.filter(
          (res: { __component: string }) =>
            res.__component === 'blocks.api-section'
        )[0];
        this.backGroundImageUrl =
          environment.imageEndPoint +
          this.heroImageSection?.ackgroundImage?.data?.attributes?.formats
            ?.large?.url;
      })
    );
  }
    ...
    this.unsubscribe$.add(
      this.courseService.getcoursesBySlug(slug).subscribe((res: any) => {
        ...
        if (typeof window !== 'undefined') {
          ...
          this.sharingUrls.linkedin = `https://www.linkedin.com/shareArticle?mini=true&url=${pageUrl}&title=${encodeURIComponent(
  ...
  getCourseData(slug: any) {
    this.unsubscribe$.add(
      this.courseService.getcoursesBySlug(slug).subscribe((res: any) => {
        ...
        this.courseData = this.coursesDetail?.data[0]?.attributes;
        this.courseCategory =
          this.coursesDetail?.data[0]?.attributes?.category?.data?.attributes?.title;
        ...
        const faqContent =
          this.courseData?.category.data?.attributes?.faqs?.faq[0];
        ...
        this.courseData.instructors?.data?.forEach(
          (element: any, index: number) => {
            ...
            if (element?.attributes?.shortDesc == '') {
              ...
              element?.attributes?.firstName == null ||
              element?.attributes?.firstName == undefined ||
              element?.attributes?.shortDesc == null ||
              element?.attributes?.shortDesc == undefined
            ) {
              facultyTitle = '';
            } else
              facultyTitle =
                ...
                '-' +
                ' ' +
                element?.attributes?.shortDesc;

            if (
              element?.attributes?.image?.data?.attributes?.url === undefined
            ) {
              imageurl = '';
            } else {
        ...
        let faqfromCourse =
          this.courseTabs.filter(
            (item: { title: string }) => item?.title.toLowerCase() === 'faq'
          ) || null;
        ...
        if (this.courseTabs != undefined || this.courseTabs.length > 0) {
          ...
          else this.courseTabs = [...this.courseTabs, ...dummyFacultyTab];
        }

        let tabcomponent =
          this.courseTabs.filter(
            (item: { index: string }) => item?.index === 'Outline'
          ) || {};

        if (
          tabcomponent != undefined ||
          (tabcomponent != null && tabcomponent[0]?.image != null) ||
          tabcomponent[0]?.image != undefined
        ) {
          if (tabcomponent[0]?.image['data'] != null)
            this.tabImageUrl =
              this.imageUrl + tabcomponent[0].image['data']['attributes'].url;
          else
            this.tabImageUrl =
              this.imageUrl + this.courseData?.image?.data?.attributes?.url;
        } else
          this.tabImageUrl =
            this.imageUrl + this.courseData?.image?.data?.attributes?.url;

        this.keywords =
          this.courseData?.keywords === null
            ? ''
            : this.courseData?.keywords?.split(',') || '';
        if (this.keywords != '') this.getAllRelatedCourses();
      })
    );
  }
      ...
      this.courseService.getAllCourses().subscribe((res: any) => {
        ...
        if (this.keywords) {
          this.keywords?.forEach((element: any) => {
            const filteredResult = coursesArray.filter(
              (item: any) =>
            ...
            filteredResult.forEach((element: any, index: number) => {
              const facultyname = this.getInstructorName(
                element?.attributes?.instructors
              );
            ...
            if (filteredResult) {
              ...
              this.relatedCourses = this.relatedCourses.filter(
                (item: any, index: number) =>
                  this.relatedCourses.indexOf(item) === index
              );
              this.relatedCourses = this.relatedCourses.filter(
                (item: any) => item.id != this.courseId
              );
            }
          });
        } else {
  ...
  addItem(item: any) {
    if (
      this.cartData.data.CartItem === undefined ||
      this.cartData.data.CartItem === null
    )
      this.cartData.data.CartItem = [];
  ...
  enrollNow2(selectedCourse: any, template: any, templateExpired: any) {
    ...
    const category =
      selectedCourse?.data[0]?.attributes['category']?.data?.attributes[
        'title'
      ] || '';
    if (
      new Date(selectedCourse.data[0].attributes['endDate']) < new Date() &&
      category == 'Live'
    ) {
    ...
    if (localStorage.getItem('token')) {
      ...
      this.courseService
        .checkAlreadyCoursePurchased(courseid, email)
        .subscribe((res: any) => {
          ...
          if (
            dts.length > 0 &&
            dts[0].attributes?.course?.data.id == courseid
          ) {
            this.isPurchased = dts[0].attributes?.course?.data.id == courseid; // course already purchased
          }
          if (this.isPurchased) this.openModal(template);
          else {
            if (this.cartId == 0 && courseid > 0) {
              ...
              let realPrice: any;
              if (
                selectedCourse.data[0].attributes.discount != null &&
                selectedCourse.data[0].attributes.discount != 0
              ) {
            ...
            if (this.cartId > 0) {
              ...
              let matchingCourse =
                this.cartData.data.CartItem.filter(
    ...
    this.cartData.data.CartItem.map((ci: any, index: any) => {
      if (ci.course != undefined) {
        if (ci.courseId > 0) {
          total +=
            (ci.course.discount ||
              ci.course.discountedPrice ||
              ci.course.discounted_price) > 0
              ? ci.course.discount * ci.qty ||
                ci.course.discountedPrice * ci.qty ||
                ci.course.discounted_price * ci.qty
              : ci.course.price * ci.qty;
        } else {
          if (
            ci.course.discounted_price != null &&
            ci.course.discounted_price > 0
          ) {
  ...
  updateCart() {
    ...
    this.unsubscribe$.add(
      this.cartService.updateCart(this.cartData, this.cartId).subscribe(
        (cartResp: any) => {
          this.getcardCount();
          this.router.navigateByUrl('/learner/shopping-cart');
        },
